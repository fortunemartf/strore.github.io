...
session_start();
$con = mysqli_connect("localhost", "solaric4", "Amit@2020#", "solaric4_kusum");
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
	...
	if($checkexists->num_rows > 0){
		...
		$mail = new PHPMailer(true);
			...
			$mail->Host       = 'mail.solarpumpsolutions.in';  // Specify main and backup SMTP servers
			$mail->SMTPAuth   = true;                                   // Enable SMTP authentication
		    $mail->Username   = 'noreply@solarpumpsolutions.in';                     // SMTP username
			$mail->Password   = 'Amit@2020';                      // SMTP password
			$mail->SMTPSecure = 'ssl';                                  // Enable TLS encryption, `ssl` also accepted
			$mail->Port       = 465;                                    // TCP port to connect to

			//Recipients
			$mail->setFrom('noreply@solarpumpsolutions.in');					########Put the Email same as Above
